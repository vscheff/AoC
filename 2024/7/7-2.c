...
int main(int argc, char **argv) {
	...
	while (getline(&line_ptr, &n, in_file) != -1) {
		...
		if (evaluate(val, nums + 1, count - 1, *nums)) { printf("%u Valid\n", line); total += val; }
		else {
			char buff[32];
			sprintf(buff, "%lu%lu\0", *nums, nums[1]);
			unsigned long concat_num = strtoul(buff, NULL, 10);
			
			if (evaluate(val, nums + 2, count - 2, concat_num)) { printf("%u Valid\n", line); total += val; }
			else {
				printf("%u Invalid\n", line);
				printf("\tvalue = %lu\n", val);
				printf("\tarray = [");
...
unsigned evaluate(unsigned long val, unsigned long *nums, size_t count, unsigned long result) {
	...
	if (count == 1) {
		...
		char buff[32];
		sprintf(buff, "%lu%lu\0", result, *nums);
		
		if (val == strtoul(buff, NULL, 10)) return 1;
	...
	return evaluate(val, nums + 1, count - 1, concat_num);
}
