...
void heapify(PATH_T *arr, size_t arr_len, size_t index) {
	...
	if (left < arr_len && arr[left].cost < arr[smallest].cost) smallest = left;
	if (right < arr_len && arr[right].cost < arr[smallest].cost) smallest = right;
...
void insert(PATH_T **arr, size_t *arr_len, size_t x, size_t y, DIR_E dir, size_t cost) {
	size_t i = *arr_len, j = get_parent(*arr_len);
	*arr = realloc(*arr, ++(*arr_len) * sizeof(**arr));
	PATH_T new_node = { { x, y }, dir, cost};
